...
const colorMap = {
    orange: "text-[#FF6B00] border-[#FF6B00]/30 bg-[#FF6B00]/10",
    ...
    return (
        ...
        >
            ...
            <div className="mb-6 flex items-center justify-between z-10 shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-[11px] font-mono text-red-500/80 uppercase tracking-widest mt-0.5">Chain Command Invoked</div>
                    </div>
                </div>
                <div className="px-3 py-1.5 rounded bg-red-500/20 border border-red-500/40 text-[10px] font-bold uppercase tracking-widest text-red-100 flex items-center gap-2 animate-pulse">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-3 pb-8 space-y-6">
                ...
                {/* Objective Banner */}
                <div className="p-5 rounded-2xl bg-[#050505] border border-white/10 shadow-inner flex flex-col items-center text-center relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-red-500/10 via-black to-black opacity-50" />
                    ...
                    <div className="flex items-center gap-4 mt-4 relative z-10 text-[11px] font-mono uppercase text-white/40 bg-white/5 px-4 py-1.5 rounded-full border border-white/10">
                        <span>Status: <span className="text-emerald-400 font-bold">{data.status}</span></span>
                        <span>•</span>
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {data.activeSubroutines.map((routine, i) => {
                            ...
                            return (
                                <div key={i} className="p-4 rounded-xl bg-[#050505] border border-white/10 flex flex-col justify-between group hover:border-white/20 transition-all relative overflow-hidden">
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <span className={cn(
                                            ...
                                            routine.status === "Running" ? "bg-white/5 border-white/10 text-white/50 animate-pulse" : "bg-emerald-500/10 border-emerald-500/20 text-emerald-400"
                                        )}>
                                            {routine.status}
                                        </span>
                                    </div>
                        ...
                        {data.globalAlerts.map((alert, i) => (
                            <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-white/5 border border-white/5">
                                {alert.type === "Success" ? (
                                    ...
                                    <div className="w-6 h-6 rounded bg-yellow-500/10 flex items-center justify-center shrink-0">
                                        <AlertTriangle className="w-3.5 h-3.5 text-yellow-500" />
                                    </div>
                                )}
                                <div className="text-xs text-white/70 font-medium">
                                    <span className={cn("font-bold mr-1", alert.type === "Success" ? "text-emerald-400" : "text-yellow-500")}>
