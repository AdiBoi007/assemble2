            ...
            <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[11px] text-zinc-300/80 font-mono uppercase tracking-widest mt-0.5">Active Execution Nodes</p>
                    </div>
                </div>
            </div>
                ...
                {agents.map((agent, i) => (
                    <div key={agent.id} className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.05] flex items-start gap-4 relative overflow-hidden group">
                        {agent.status === "running" && (
                            <div className="absolute top-0 right-0 w-32 h-32 bg-zinc-800/5 rounded-full blur-[40px] -translate-y-1/2 translate-x-1/2 pointer-events-none animate-pulse" />
                        )}
                        ...
                        <div className="flex-1 min-w-0 pt-1 relative z-10">
                            <div className="flex items-center justify-between mb-1">
                                ...
                                {agent.status === "running" ? (
                                    <RefreshCw className="w-3 h-3 text-zinc-300 animate-spin drop-shadow-[0_0_5px_rgba(255,107,0,0.8)]" />
                                ) : (
                            ...
                            {agent.status === "running" && (
                                <div className="mt-3 flex items-center gap-2">
                                    <div className="w-1 h-1 rounded-full bg-zinc-800 animate-ping" />
                                    <span className="text-[9px] uppercase tracking-widest text-zinc-300/80 font-bold">Executing</span>
                                </div>
                            )}
                        </div>
                    </div>
