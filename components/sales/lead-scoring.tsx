        ...
        >
            ...
            <div className="mb-6 flex items-center justify-between z-10 shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-[11px] font-mono text-indigo-400/80 uppercase tracking-widest mt-0.5">Machine Learning Model Active</div>
                    </div>
                </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-3 pb-8 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 rounded-xl bg-[#FF6B00]/5 border border-[#FF6B00]/20 flex flex-col justify-between relative overflow-hidden group">
                        <div className="absolute top-0 right-0 w-24 h-24 bg-[#FF6B00]/10 rounded-full blur-[30px] -translate-y-1/2 translate-x-1/2 pointer-events-none" />
                    ...
                    <div className="p-4 rounded-xl bg-[#050505] border border-white/10 flex flex-col justify-between shadow-inner relative overflow-hidden group">
                        <div className="absolute bottom-0 left-0 w-24 h-24 bg-emerald-500/10 rounded-full blur-[30px] translate-y-1/2 -translate-x-1/2 pointer-events-none" />
                        <div className="flex items-center gap-2 mb-4 relative z-10">
                            <Zap className="w-4 h-4 text-emerald-400" />
                            <span className="text-[10px] font-bold tracking-widest uppercase text-white/50">Conversion Probability</span>
                        </div>
                        <span className="text-3xl font-black text-emerald-400 tracking-tight relative z-10">{data.conversionProbability}</span>
                ...
                <div className="p-5 rounded-2xl bg-[#050505] border border-white/10 shadow-inner">
                    <div className="flex items-center justify-between mb-5">
                        ...
                        <button className="text-[10px] font-bold tracking-widest uppercase text-[#FF6B00] hover:text-white transition-colors flex items-center gap-1">
                    ...
                    <div className="space-y-3">
                        {data.rankedLeads.map((lead, i) => (
                            <div key={i} className="flex flex-col p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-indigo-500/30 transition-colors group gap-3 relative overflow-hidden">
                                {i === 0 && (
                                    <div className="absolute top-0 left-0 w-1 h-full bg-[#FF6B00] shadow-[0_0_15px_rgba(255,107,0,0.6)]" />
                                )}

                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div className="flex items-center gap-4">
                                        <div className="flex flex-col items-center justify-center shrink-0">
                                            <div className="w-12 h-12 rounded-full border-[3px] flex items-center justify-center font-black text-lg bg-[#050505]"
                                                style={{ borderColor: lead.score > 90 ? '#FF6B00' : lead.score > 80 ? '#6366f1' : '#3f3f46', color: lead.score > 90 ? '#FF6B00' : lead.score > 80 ? '#818cf8' : '#a1a1aa' }}>
                                                {lead.score}
                                            </div>
                                    ...
                                    <div className="flex flex-wrap md:flex-col md:items-end gap-1.5 md:gap-1 pl-16 md:pl-0">
                                        {lead.triggers.map((trigger, j) => (
                                            <span key={j} className="text-[9px] uppercase font-bold tracking-widest px-2 py-0.5 rounded border bg-indigo-500/10 border-indigo-500/20 text-indigo-300">
                                                {trigger}
                                            </span>
                                        ))}
                                    </div>
