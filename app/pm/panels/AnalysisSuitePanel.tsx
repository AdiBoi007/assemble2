            ...
            <div className="px-6 py-5 border-b border-white/[0.06] flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[11px] text-white/40 uppercase tracking-wider">Decisions · Communication · Benchmarks</p>
                    </div>
                </div>
...
function DecisionTab() {
    ...
    return (
        <div className="space-y-5">
            <div>
                <h3 className="text-sm font-semibold text-white">{d.candidateName} — Decision Trace</h3>
                <p className="text-[11px] text-white/40 mt-0.5">{d.taskTitle}</p>
            </div>
            ...
            <div className="grid grid-cols-4 gap-2">
                {[
                    ...
                    <div key={s.label} className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-2.5 text-center">
            ...
            <div className="space-y-2">
                {d.steps.map((step, i) => {
                    ...
                    return (
                        <div key={step.id} className="rounded-xl border border-white/[0.08] bg-white/[0.02] p-3">
                            <div className="flex items-center gap-2 mb-1.5">
                                <span className="w-5 h-5 rounded-full bg-white/[0.08] flex items-center justify-center text-[10px] font-bold text-white/60">{i + 1}</span>
                                <span className="text-[11px] text-white/70 font-medium flex-1">{step.step}</span>
                                <span className={`text-[9px] font-medium px-1.5 py-0.5 rounded ${qc.color}`}>{qc.label}</span>
                            </div>
                            <p className="text-[10px] text-white/50 mb-1.5">{step.description}</p>
                            <p className="text-[10px] text-white/30 italic">{step.reasoning}</p>
                        </div>
                    )
                })}
            </div>
        ...
        <div className="space-y-5">
            ...
            <div className="space-y-2">
                {c.dimensions.map(dim => (
                    <div key={dim.dimension} className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-3">
                        <div className="flex items-center justify-between mb-1.5">
                            <span className="text-[11px] text-white/70 font-medium">{dim.icon} {dim.dimension}</span>
            ...
            {/* Sample Quotes */}
            <div>
                <p className="text-[10px] text-white/30 uppercase tracking-wider font-medium mb-2">Sample Quotes</p>
                {c.sampleQuotes.map((sq, i) => (
                    <div key={i} className={`rounded-lg border p-3 mb-2 ${quoteColors[sq.rating]}`}>
                        <p className="text-[11px] text-white/60 italic">&ldquo;{sq.quote}&rdquo;</p>
                        <p className="text-[9px] text-white/30 mt-1">{sq.context} — {sq.rating}</p>
                    </div>
                ))}
            </div>
...
function BenchmarkTab() {
    ...
    return (
        <div className="space-y-5">
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="text-sm font-semibold text-white">{b.candidateName} — Work Benchmark</h3>
                    <p className="text-[11px] text-white/40 mt-0.5">{b.taskTitle}</p>
                </div>
            ...
            <div className="space-y-2">
                {b.metrics.map(m => {
                    ...
                    return (
                        <div key={m.metric} className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-3">
                            <div className="flex items-center justify-between mb-1">
                                <span className="text-[11px] text-white/70 font-medium">{m.metric}</span>
                                <span className="text-[11px] text-white/60 font-mono font-bold">{m.candidateScore}{m.unit}</span>
                            </div>
                            <div className="relative h-2 rounded-full bg-white/[0.06] mb-1.5">
                                {/* Percentile markers */}
                                <div className="absolute top-0 h-full w-px bg-white/10" style={{ left: '0%' }} />
                                <div className="absolute top-0 h-full w-px bg-white/10" style={{ left: '38%' }} />
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="text-[8px] text-white/20">P50</span>
                                <span className="text-[8px] text-white/20">P75</span>
                                <span className="text-[8px] text-white/20">P90</span>
                            </div>
                            <p className="text-[9px] text-white/30 mt-1">{m.interpretation}</p>
                        </div>
                    )
                })}
            </div>
            ...
            {/* Comparison */}
            <div>
                <p className="text-[10px] text-white/30 uppercase tracking-wider font-medium mb-2">vs Previous Candidates</p>
                <div className="space-y-1">
                    {b.comparisons.map((c, i) => (
                        <div key={i} className="flex items-center gap-2">
                            <span className="text-[10px] text-white/50 w-28 truncate">{c.name}</span>
                            ...
                            <span className="text-[9px] text-white/30 font-mono w-6">{c.score}</span>
...
function CheatTab() {
    ...
    return (
        <div className="space-y-5">
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="text-sm font-semibold text-white">{r.candidateName} — Cheat Detection</h3>
                    <p className="text-[11px] text-white/40 mt-0.5">{r.taskTitle}</p>
                </div>
            ...
            <div className="rounded-xl border border-white/[0.08] bg-white/[0.02] p-3">
                <p className="text-[11px] text-white/50">{r.summary}</p>
            </div>

            {/* Signals */}
            <div>
                <p className="text-[10px] text-white/30 uppercase tracking-wider font-medium mb-2">Detection Signals</p>
                {r.signals.map(sig => {
                    ...
                    return (
                        <div key={sig.id} className={`rounded-xl border p-3 mb-2 ${sev.color}`}>
                            <div className="flex items-center gap-2 mb-1.5">
                                ...
                                <span className="text-[11px] text-white/70 font-medium flex-1">{sig.description}</span>
                                <span className={`text-[9px] font-medium px-1.5 py-0.5 rounded ${sev.color}`}>{sev.label}</span>
                            </div>
                            <p className="text-[10px] text-white/40 mb-1.5">{sig.evidence}</p>
                            <div className="flex items-center justify-between">
                                <span className="text-[9px] text-white/30">Confidence: {sig.confidence}%</span>
